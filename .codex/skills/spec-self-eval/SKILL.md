...
- **Feature** (required): the subdirectory name under `.specs/`, e.g. `query-api`. If the user does not name one, list the candidate directories under `.specs/` and ask which to evaluate. If exactly one feature directory exists, use it without asking.
